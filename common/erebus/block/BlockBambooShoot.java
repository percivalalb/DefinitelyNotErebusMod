...
public class BlockBambooShoot extends BlockFlower implements IPlantable {
	public static byte calculateBambooHappiness(World world, int x, int y, int z, int blockID){
		...
		if (soilValue!=null)happiness*=soilValue;
	...
	public void updateTick(World world, int x, int y, int z, Random rand){
		if (!world.isRemote){
			...
			if (rand.nextInt(5)<=1 && happiness>rand.nextInt(35)){
				...
				/*if ((meta&7)<7)world.setBlockMetadataWithNotify(x,y,z,meta+1,4);
				else */world.setBlock(x,y,z,ModBlocks.bambooCrop.blockID,(meta>=8?8:0)+(int)Math.min(7D,2D+(happiness/7D)+(meta>=8?0.5D:0D)+Math.abs((happiness/10D+(meta>=8?0.6D:0D))*rand.nextGaussian())),3);
			}
		}
	}
	...
	public void onBonemeal(BonemealEvent e){
		if (!e.world.isRemote&&e.ID==blockID){
			...
			if (meta<7){
				e.world.setBlockMetadataWithNotify(e.X,e.Y,e.Z,meta+9,4); // prevent autogrowing full bamboo
