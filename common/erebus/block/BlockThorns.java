...
public class BlockThorns extends Block implements IShearable {
			...
			if (j1 == 1 && par3 < 255 && par1World.isAirBlock(par2, par3 + 1, par4)) {
				...
				if (j1 >= 2 && j1 <= 5 && (i1 & 1 << k1) == 0) {
					...
					if (l1 != 0 && Block.blocksList[l1] != null) {
						...
						this.canBePlacedOn(par1World.getBlockId(par2 + Direction.offsetX[i2], par3, par4 + Direction.offsetZ[i2]))) {
						...
						this.canBePlacedOn(par1World.getBlockId(par2 + Direction.offsetX[j2], par3, par4 + Direction.offsetZ[j2]))) {
	...
	/*
	 * public void registerIcons(IconRegister par1IconRegister) { this.blockIcon
	 * = par1IconRegister.registerIcon("erebus:thorns"); }
	 */
}
