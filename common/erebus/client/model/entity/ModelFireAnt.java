package erebus.client.model.entity;

import net.minecraft.client.model.ModelBase;
import net.minecraft.client.model.ModelRenderer;
import net.minecraft.entity.Entity;
import net.minecraft.util.MathHelper;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

@SideOnly(Side.CLIENT)
public class ModelFireAnt extends ModelBase {
	ModelRenderer Thx;
	ModelRenderer ThxTop;
	ModelRenderer ThxS;
	ModelRenderer ThxToAb;
	ModelRenderer Ab;
	ModelRenderer AbF;
	ModelRenderer AbSide;
	ModelRenderer AbTop;
	ModelRenderer AbBack;
	ModelRenderer Neck;
	ModelRenderer HeadA;
	ModelRenderer HeadB;
	ModelRenderer RMandibleA;
	ModelRenderer RMandibleB;
	ModelRenderer LMandibleA;
	ModelRenderer LMandibleB;
	ModelRenderer Eyes;
	ModelRenderer AntLS;
	ModelRenderer AntLE;
	ModelRenderer AntRS;
	ModelRenderer AntRE;
	ModelRenderer LBLA;
	ModelRenderer LBLB;
	ModelRenderer LBLC;
	ModelRenderer LBLD;
	ModelRenderer LMLA;
	ModelRenderer LMLB;
	ModelRenderer LMLC;
	ModelRenderer LMLD;
	ModelRenderer LFLA;
	ModelRenderer LFLB;
	ModelRenderer LFLC;
	ModelRenderer LFLD;
	ModelRenderer RFLA;
	ModelRenderer RFLB;
	ModelRenderer RFLC;
	ModelRenderer RFLD;
	ModelRenderer RMLA;
	ModelRenderer RMLB;
	ModelRenderer RMLC;
	ModelRenderer RMLD;
	ModelRenderer RBLA;
	ModelRenderer RBLB;
	ModelRenderer RBLC;
	ModelRenderer RBLD;


	public ModelFireAnt() {
		textureWidth = 64;
		textureHeight = 128;

		Thx = new ModelRenderer(this, 14, 13);
		Thx.addBox(-3.5F, -3.5F, 0F, 7, 7, 9);
		Thx.setRotationPoint(0F, 17F, 0F);
		Thx.setTextureSize(64, 128);
		Thx.mirror = true;
		setRotation(Thx, 0F, 0F, 0F);
		ThxTop = new ModelRenderer(this, 21, 30);
		ThxTop.addBox(-2.5F, -4.5F, 1F, 5, 1, 7);
		ThxTop.setRotationPoint(0F, 17F, 0F);
		ThxTop.setTextureSize(64, 128);
		ThxTop.mirror = true;
		setRotation(ThxTop, 0F, 0F, 0F);
		ThxS = new ModelRenderer(this, 15, 39);
		ThxS.addBox(-4.5F, -2.5F, 1F, 9, 5, 7);
		ThxS.setRotationPoint(0F, 17F, 0F);
		ThxS.setTextureSize(64, 128);
		ThxS.mirror = true;
		setRotation(ThxS, 0F, 0F, 0F);
		ThxToAb = new ModelRenderer(this, 27, 52);
		ThxToAb.addBox(-1.5F, -1.5F, 0F, 3, 3, 1);
		ThxToAb.setRotationPoint(0F, 17F, 9F);
		ThxToAb.setTextureSize(64, 128);
		ThxToAb.mirror = true;
		setRotation(ThxToAb, 0F, 0F, 0F);
		Ab = new ModelRenderer(this, 9, 100);
		Ab.addBox(-5.5F, -4.5F, 0F, 11, 9, 12);
		Ab.setRotationPoint(0F, 17F, 11F);
		Ab.setTextureSize(64, 128);
		Ab.mirror = true;
		setRotation(Ab, 0F, 0F, 0F);
		AbF = new ModelRenderer(this, 23, 57);
		AbF.addBox(-3.5F, -3.5F, -1F, 7, 7, 1);
		AbF.setRotationPoint(0F, 17F, 11F);
		AbF.setTextureSize(64, 128);
		AbF.mirror = true;
		setRotation(AbF, 0F, 0F, 0F);
		AbSide = new ModelRenderer(this, 10, 66);
		AbSide.addBox(-6.5F, -2.5F, 2F, 13, 5, 8);
		AbSide.setRotationPoint(0F, 17F, 11F);
		AbSide.setTextureSize(64, 128);
		AbSide.mirror = true;
		setRotation(AbSide, 0F, 0F, 0F);
		AbTop = new ModelRenderer(this, 15, 80);
		AbTop.addBox(-4F, -5.5F, 2F, 8, 11, 8);
		AbTop.setRotationPoint(0F, 17F, 11F);
		AbTop.setTextureSize(64, 128);
		AbTop.mirror = true;
		setRotation(AbTop, 0F, 0F, 0F);
		AbBack = new ModelRenderer(this, 22, 122);
		AbBack.addBox(-3.5F, -2.5F, 12F, 7, 5, 1);
		AbBack.setRotationPoint(0F, 17F, 11F);
		AbBack.setTextureSize(64, 128);
		AbBack.mirror = true;
		setRotation(AbBack, 0F, 0F, 0F);
		Neck = new ModelRenderer(this, 0, 12);
		Neck.addBox(-1.5F, -1.5F, 0F, 3, 3, 3);
		Neck.setRotationPoint(0F, 17F, -3F);
		Neck.setTextureSize(64, 128);
		Neck.mirror = true;
		setRotation(Neck, 0F, 0F, 0F);
		HeadA = new ModelRenderer(this, 21, 0);
		HeadA.addBox(-2F, -0.5F, -4F, 4, 5, 6);
		HeadA.setRotationPoint(0F, 17F, -1F);
		HeadA.setTextureSize(64, 128);
		HeadA.mirror = true;
		setRotation(HeadA, -1F, 0F, 0F);
		HeadB = new ModelRenderer(this, 0, 0);
		HeadB.addBox(-3F, -0.5F, -3F, 6, 7, 4);
		HeadB.setRotationPoint(0F, 17F, -1F);
		HeadB.setTextureSize(64, 128);
		HeadB.mirror = true;
		setRotation(HeadB, -1F, 0F, 0F);
		RMandibleA = new ModelRenderer(this, 52, 0);
		RMandibleA.addBox(-3F, 5.5F, -2F, 1, 3, 1);
		RMandibleA.setRotationPoint(0F, 17F, -1F);
		RMandibleA.setTextureSize(64, 128);
		RMandibleA.mirror = true;
		setRotation(RMandibleA, -1F, 0F, 0F);
		RMandibleB = new ModelRenderer(this, 52, 9);
		RMandibleB.addBox(-2F, 5.5F, -2F, 1, 4, 1);
		RMandibleB.setRotationPoint(0F, 17F, -1F);
		RMandibleB.setTextureSize(64, 128);
		RMandibleB.mirror = true;
		setRotation(RMandibleB, -1F, 0F, 0F);
		LMandibleA = new ModelRenderer(this, 47, 0);
		LMandibleA.addBox(2F, 5.5F, -2F, 1, 3, 1);
		LMandibleA.setRotationPoint(0F, 17F, -1F);
		LMandibleA.setTextureSize(64, 128);
		LMandibleA.mirror = true;
		setRotation(LMandibleA, -1F, 0F, 0F);
		LMandibleB = new ModelRenderer(this, 47, 9);
		LMandibleB.addBox(1F, 5.5F, -2F, 1, 4, 1);
		LMandibleB.setRotationPoint(0F, 17F, -1F);
		LMandibleB.setTextureSize(64, 128);
		LMandibleB.mirror = true;
		setRotation(LMandibleB, -1F, 0F, 0F);
		Eyes = new ModelRenderer(this, 0, 30);
		Eyes.addBox(-4F, 1.5F, -2F, 8, 2, 2);
		Eyes.setRotationPoint(0F, 17F, -1F);
		Eyes.setTextureSize(64, 128);
		Eyes.mirror = true;
		setRotation(Eyes,-1F, 0F, 0F);
		AntLS = new ModelRenderer(this, 42, 6);
		AntLS.addBox(3F, 4.5F, -3F, 3, 1, 1);
		AntLS.setRotationPoint(0F, 17F, -1F);
		AntLS.setTextureSize(64, 128);
		AntLS.mirror = true;
		setRotation(AntLS, -1F, 0F, 0.1745329F);
		AntLE = new ModelRenderer(this, 42, 0);
		AntLE.addBox(6F, 5.5F, -3F, 1, 4, 1);
		AntLE.setRotationPoint(0F, 17F, -1F);
		AntLE.setTextureSize(64, 128);
		AntLE.mirror = true;
		setRotation(AntLE, -1F, 0F, 0.1745329F);
		AntRS = new ModelRenderer(this, 53, 6);
		AntRS.addBox(-6F, 4.5F, -3F, 3, 1, 1);
		AntRS.setRotationPoint(0F, 17F, -1F);
		AntRS.setTextureSize(64, 128);
		AntRS.mirror = true;
		setRotation(AntRS, -1F, 0F, -0.1745329F);
		AntRE = new ModelRenderer(this, 57, 0);
		AntRE.addBox(-7F, 5.5F, -3F, 1, 4, 1);
		AntRE.setRotationPoint(0F, 17F, -1F);
		AntRE.setTextureSize(64, 128);
		AntRE.mirror = true;
		setRotation(AntRE, -1F, 0F, -0.1745329F);
		LBLA = new ModelRenderer(this, 0, 95);
		LBLA.addBox(0F, -1F, -1F, 4, 2, 2);
		LBLA.setRotationPoint(4F, 17F, 8F);
		LBLA.setTextureSize(64, 128);
		LBLA.mirror = true;
		setRotation(LBLA, 0.25F, -0.7F, -0.5F);
		LBLB = new ModelRenderer(this, 0, 88);
		LBLB.addBox(3F, 0F, -1F, 2, 4, 2);
		LBLB.setRotationPoint(4F, 17F, 8F);
		LBLB.setTextureSize(64, 128);
		LBLB.mirror = true;
		setRotation(LBLB, 0.25F, -0.7F, -0.5F);
		LBLC = new ModelRenderer(this, 0, 82);
		LBLC.addBox(1.5F, 4.5F, -0.5F, 2, 4, 1);
		LBLC.setRotationPoint(4F, 17F, 8F);
		LBLC.setTextureSize(64, 128);
		LBLC.mirror = true;
		setRotation(LBLC, 0.5F, -0.6F, -0.95F);
		LBLD = new ModelRenderer(this, 0, 76);
		LBLD.addBox(0.5F, 8F, -0.5F, 1, 4, 1);
		LBLD.setRotationPoint(4F, 17F, 8F);
		LBLD.setTextureSize(64, 128);
		LBLD.mirror = true;
		setRotation(LBLD, 0.64F, -0.7F, -1.27F);
		LMLA = new ModelRenderer(this, 0, 95);
		LMLA.addBox(0F, -1F, -1F, 4, 2, 2);
		LMLA.setRotationPoint(4F, 17F, 5F);
		LMLA.setTextureSize(64, 128);
		LMLA.mirror = true;
		setRotation(LMLA, 0F, 0F, -0.3490659F);
		LMLB = new ModelRenderer(this, 0, 88);
		LMLB.addBox(3F, 0F, -1F, 2, 4, 2);
		LMLB.setRotationPoint(4F, 17F, 5F);
		LMLB.setTextureSize(64, 128);
		LMLB.mirror = true;
		setRotation(LMLB, 0F, 0F, -0.3490659F);
		LMLC = new ModelRenderer(this, 0, 82);
		LMLC.addBox(1.5F, 4.5F, -0.5F, 2, 4, 1);
		LMLC.setRotationPoint(4F, 17F, 5F);
		LMLC.setTextureSize(64, 128);
		LMLC.mirror = true;
		setRotation(LMLC, 0F, 0F, -0.6981317F);
		LMLD = new ModelRenderer(this, 0, 76);
		LMLD.addBox(0.5F, 8F, -0.5F, 1, 4, 1);
		LMLD.setRotationPoint(4F, 17F, 5F);
		LMLD.setTextureSize(64, 128);
		LMLD.mirror = true;
		setRotation(LMLD, 0F, 0F, -0.8726646F);
		LFLA = new ModelRenderer(this, 0, 95);
		LFLA.addBox(0F, -1F, -1F, 4, 2, 2);
		LFLA.setRotationPoint(4F, 17F, 2F);
		LFLA.setTextureSize(64, 128);
		LFLA.mirror = true;
		setRotation(LFLA, -0.25F, 0.7F, -0.5F);
		LFLB = new ModelRenderer(this, 0, 88);
		LFLB.addBox(3F, 0F, -1F, 2, 4, 2);
		LFLB.setRotationPoint(4F, 17F, 2F);
		LFLB.setTextureSize(64, 128);
		LFLB.mirror = true;
		setRotation(LFLB, -0.25F, 0.7F, -0.5F);
		LFLC = new ModelRenderer(this, 0, 82);
		LFLC.addBox(1.5F, 4.5F, -0.5F, 2, 4, 1);
		LFLC.setRotationPoint(4F, 17F, 2F);
		LFLC.setTextureSize(64, 128);
		LFLC.mirror = true;
		setRotation(LFLC, -0.5F, 0.6F, -0.95F);
		LFLD = new ModelRenderer(this, 0, 76);
		LFLD.addBox(0.5F, 8F, -0.5F, 1, 4, 1);
		LFLD.setRotationPoint(4F, 17F, 2F);
		LFLD.setTextureSize(64, 128);
		LFLD.mirror = true;
		setRotation(LFLD, -0.64F, 0.7F, -1.27F);
		RFLA = new ModelRenderer(this, 0, 95);
		RFLA.addBox(-4F, -1F, -1F, 4, 2, 2);
		RFLA.setRotationPoint(-4F, 17F, 2F);
		RFLA.setTextureSize(64, 128);
		RFLA.mirror = true;
		setRotation(RFLA, -0.25F, -0.7F, 0.5F);
		RFLB = new ModelRenderer(this, 0, 88);
		RFLB.addBox(-5F, 0F, -1F, 2, 4, 2);
		RFLB.setRotationPoint(-4F, 17F, 2F);
		RFLB.setTextureSize(64, 128);
		RFLB.mirror = true;
		setRotation(RFLB, -0.25F, -0.7F, 0.5F);
		RFLC = new ModelRenderer(this, 0, 82);
		RFLC.addBox(-3.5F, 4.5F, -0.5F, 2, 4, 1);
		RFLC.setRotationPoint(-4F, 17F, 2F);
		RFLC.setTextureSize(64, 128);
		RFLC.mirror = true;
		setRotation(RFLC, -0.5F, -0.6F, 0.95F);
		RFLD = new ModelRenderer(this, 0, 76);
		RFLD.addBox(-1.5F, 8F, -0.5F, 1, 4, 1);
		RFLD.setRotationPoint(-4F, 17F, 2F);
		RFLD.setTextureSize(64, 128);
		RFLD.mirror = true;
		setRotation(RFLD, -0.64F, -0.7F, 1.27F);
		RMLA = new ModelRenderer(this, 0, 95);
		RMLA.addBox(-4F, -1F, -1F, 4, 2, 2);
		RMLA.setRotationPoint(-4F, 17F, 5F);
		RMLA.setTextureSize(64, 128);
		RMLA.mirror = true;
		setRotation(RMLA, 0F, 0F, 0.3490659F);
		RMLB = new ModelRenderer(this, 0, 88);
		RMLB.addBox(-5F, 0F, -1F, 2, 4, 2);
		RMLB.setRotationPoint(-4F, 17F, 5F);
		RMLB.setTextureSize(64, 128);
		RMLB.mirror = true;
		setRotation(RMLB, 0F, 0F, 0.3490659F);
		RMLC = new ModelRenderer(this, 0, 82);
		RMLC.addBox(-3.5F, 4.5F, -0.5F, 2, 4, 1);
		RMLC.setRotationPoint(-4F, 17F, 5F);
		RMLC.setTextureSize(64, 128);
		RMLC.mirror = true;
		setRotation(RMLC, 0F, 0F, 0.6981317F);
		RMLD = new ModelRenderer(this, 0, 76);
		RMLD.addBox(-1.5F, 8F, -0.5F, 1, 4, 1);
		RMLD.setRotationPoint(-4F, 17F, 5F);
		RMLD.setTextureSize(64, 128);
		RMLD.mirror = true;
		setRotation(RMLD, 0F, 0F, 0.8726646F);
		RBLA = new ModelRenderer(this, 0, 95);
		RBLA.addBox(-4F, -1F, -1F, 4, 2, 2);
		RBLA.setRotationPoint(-4F, 17F, 8F);
		RBLA.setTextureSize(64, 128);
		RBLA.mirror = true;
		setRotation(RBLA, 0.25F, 0.7F, 0.5F);
		RBLB = new ModelRenderer(this, 0, 88);
		RBLB.addBox(-5F, 0F, -1F, 2, 4, 2);
		RBLB.setRotationPoint(-4F, 17F, 8F);
		RBLB.setTextureSize(64, 128);
		RBLB.mirror = true;
		setRotation(RBLB, 0.25F, 0.7F, 0.5F);
		RBLC = new ModelRenderer(this, 0, 82);
		RBLC.addBox(-3.5F, 4.5F, -0.5F, 2, 4, 1);
		RBLC.setRotationPoint(-4F, 17F, 8F);
		RBLC.setTextureSize(64, 128);
		RBLC.mirror = true;
		setRotation(RBLC, 0.5F, 0.6F, 0.95F);
		RBLD = new ModelRenderer(this, 0, 76);
		RBLD.addBox(-1.5F, 8F, -0.5F, 1, 4, 1);
		RBLD.setRotationPoint(-4F, 17F, 8F);
		RBLD.setTextureSize(64, 128);
		RBLD.mirror = true;
		setRotation(RBLD, 0.64F, 0.7F, 1.27F);

	}

	@Override
	public void render(Entity par1Entity, float par2, float par3, float par4, float par5, float par6, float par7)
	{
		super.render(par1Entity, par2, par3, par4, par5, par6, par7);
		setRotationAngles(par2, par3, par4, par5, par6, par7, par1Entity);
		Thx.render(par7);
		ThxTop.render(par7);
		ThxS.render(par7);
		ThxToAb.render(par7);
		Ab.render(par7);
		AbF.render(par7);
		AbSide.render(par7);
		AbTop.render(par7);
		AbBack.render(par7);
		Neck.render(par7);
		HeadA.render(par7);
		HeadB.render(par7);
		RMandibleA.render(par7);
		RMandibleB.render(par7);
		LMandibleA.render(par7);
		LMandibleB.render(par7);
		Eyes.render(par7);
		AntLS.render(par7);
		AntLE.render(par7);
		AntRS.render(par7);
		AntRE.render(par7);
		LBLA.render(par7);
		LBLB.render(par7);
		LBLC.render(par7);
		LBLD.render(par7);
		LMLA.render(par7);
		LMLB.render(par7);
		LMLC.render(par7);
		LMLD.render(par7);
		LFLA.render(par7);
		LFLB.render(par7);
		LFLC.render(par7);
		LFLD.render(par7);
		RFLA.render(par7);
		RFLB.render(par7);
		RFLC.render(par7);
		RFLD.render(par7);
		RMLA.render(par7);
		RMLB.render(par7);
		RMLC.render(par7);
		RMLD.render(par7);
		RBLA.render(par7);
		RBLB.render(par7);
		RBLC.render(par7);
		RBLD.render(par7);
	}

	private void setRotation(ModelRenderer model, float x, float y, float z) {
		model.rotateAngleX = x;
		model.rotateAngleY = y;
		model.rotateAngleZ = z;
	}

	@Override
	public void setRotationAngles(float par1, float par2, float par3, float par4, float par5, float par6, Entity par7Entity) {
		HeadA.rotateAngleY = par4 / (180F / (float)Math.PI);
		HeadB.rotateAngleY = par4 / (180F / (float)Math.PI);
		AntLS.rotateAngleY = par4 / (180F / (float)Math.PI);
		AntLE.rotateAngleY = par4 / (180F / (float)Math.PI);
		AntRS.rotateAngleY = par4 / (180F / (float)Math.PI);
		AntRE.rotateAngleY = par4 / (180F / (float)Math.PI);
		Eyes.rotateAngleY = par4 / (180F / (float)Math.PI);
		RMandibleA.rotateAngleY = par4 / (180F / (float)Math.PI);
		RMandibleB.rotateAngleY = par4 / (180F / (float)Math.PI);
		LMandibleA.rotateAngleY = par4 / (180F / (float)Math.PI);
		LMandibleB.rotateAngleY = par4 / (180F / (float)Math.PI);
		HeadA.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		HeadB.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		AntLS.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		AntLE.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		AntRS.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		AntRE.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		Eyes.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		RMandibleA.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		RMandibleB.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		LMandibleA.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		LMandibleB.rotateAngleX = par5 / (180F / (float)Math.PI)-1F;
		LBLA.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 +0.25F;
		LBLB.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 +0.25F;
		LBLC.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 +0.5F;
		LBLD.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 +0.64F;
		LMLA.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2;
		LMLB.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2;
		LMLC.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2;
		LMLD.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2;
		LFLA.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 -0.25F;
		LFLB.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 -0.25F;
		LFLC.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 -0.5F;
		LFLD.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2 -0.64F;
		RBLA.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 +0.25F;
		RBLB.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 +0.25F;
		RBLC.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 +0.5F;
		RBLD.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 +0.64F;
		RMLA.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2;
		RMLB.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2;
		RMLC.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2;
		RMLD.rotateAngleX = MathHelper.cos(par1 * 2.0F + (float)Math.PI) * 0.7F * par2;
		RFLA.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 -0.25F;
		RFLB.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 -0.25F;
		RFLC.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 -0.5F;
		RFLD.rotateAngleX = MathHelper.cos(par1 * 2.0F) * 0.7F * par2 -0.64F;
	}
}