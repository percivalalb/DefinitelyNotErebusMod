...
public class CreativeTabErebus extends CreativeTabs{
	...
	public void add(Block...blocks){
		for(Block block:blocks)blockList.add((short)block.blockID);
	}
	
	public void add(Item...items){
		for(Item item:items)blockList.add((short)item.itemID);
	}
	...
	public void displayAllReleventItems(List list){
		for(short s:blockList){
			...
			if (item!=null)item.getSubItems(item.itemID,this,list);
		}
	}
}
