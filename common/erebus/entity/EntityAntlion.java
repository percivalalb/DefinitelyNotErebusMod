...
public class EntityAntlion extends EntityMob {
	...
	return worldObj.getBlockId(var1, var2 - 1, var3) == Block.sand.blockID;
    }
	...
	getEntityAttribute(SharedMonsterAttributes.movementSpeed).setAttribute(
		1.7D);
	if (!worldObj.isRemote && entityToAttack == null && isOnSand())
	    yOffset = -1;
	else
	    yOffset = 0;
    }
    ...
    @ForgeSubscribe
    public void onWallDamage(LivingAttackEvent par1) {
	if (par1.entity instanceof EntityAntlion)
	    if (par1.source.equals(DamageSource.inWall)
		    || par1.source.equals(DamageSource.drown))
		par1.setCanceled(true);
    }
}