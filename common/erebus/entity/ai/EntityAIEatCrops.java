...
public class EntityAIEatCrops extends EntityAIBase
{
	...
	{
		return(theEntity.worldObj.getBlockId(PlantX, PlantY, PlantZ) != Block.tallGrass.blockID) ? false : !theEntity.getNavigator().noPath()
		|| (theEntity.worldObj.getBlockId(PlantX, PlantY, PlantZ) != Block.crops.blockID)? false : !theEntity.getNavigator().noPath();
	}
	/**
	 * Updates the task
	 */
	...
	public void updateTask()
	{
		...
		theEntity.getLookHelper().setLookPosition(PlantX, PlantY, PlantZ, 50.0F, 8.0F);//last two params are how quickly look can snap
		...
		ticksSpent++;
		if(theEntity.boundingBox.maxY >= blockbounds.minY && theEntity.boundingBox.minY <= blockbounds.maxY
		&& theEntity.boundingBox.maxX >= blockbounds.minX && theEntity.boundingBox.minX <= blockbounds.maxX
