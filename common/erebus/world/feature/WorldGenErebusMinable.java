...
public class WorldGenErebusMinable extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, int x, int y, int z) {
		...
		for (int attempt = 0, placed = 0; attempt <= numberOfBlocks && placed <= realNumberOfBlocks; ++attempt) {
			double d6 = d0 + (d1 - d0) * placed / this.numberOfBlocks;
			double d7 = d4 + (d5 - d4) * placed / this.numberOfBlocks;
			double d8 = d2 + (d3 - d2) * placed / this.numberOfBlocks;
			double d9 = rand.nextDouble() * this.numberOfBlocks / 16.0D;
			double d10 = (MathHelper.sin(placed * (float) Math.PI / this.numberOfBlocks) + 1.0F) * d9 + 1.0D;
			double d11 = (MathHelper.sin(placed * (float) Math.PI / this.numberOfBlocks) + 1.0F) * d9 + 1.0D;
