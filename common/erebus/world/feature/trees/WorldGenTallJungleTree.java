...
public class WorldGenTallJungleTree extends WorldGenerator{
	...
	public boolean generate(World world, Random random, int x, int y, int z){
		...
		for(int branch=0; branch<branchAmount; branch++){
			...
			// + shaped leaves
			for(int a=0; a<2; a++){
				for(int b=0; b<4; b++)world.setBlock(x+offsetX[b],y+branch*3+a*2,z+offsetZ[b],Block.leaves.blockID,3,3);
			}
			// circular leaves mid-branch
			for(int a=0; a<2; a++){
				for(int b=0; b<2; b++)world.setBlock(x-1+a*2,y+branch*3+1,z-1+b*2,Block.leaves.blockID,3,3);
			}
