...
public class MapGenErebusRavine extends MapGenBase{
	...
	protected void generateRavine(long seed, int x, int z, byte[] blocks, double par6, double par8, double seed0, float seed2, float seed3, float seed4, int seed5, int seed6, double seed7){
		...
		double blockCoordX=(double)(x*16+8);
		double blockCoordZ=(double)(z*16+8);
		float f3=0.0F;
		float f4=0.0F;
		...
		for(; seed5<seed6; ++seed5){
			double d6=1.5D+(double)(MathHelper.sin((float)seed5*(float)Math.PI/(float)seed6)*seed2*1.0F);
			double d7=d6*seed7;
			...
			par6+=(double)(MathHelper.cos(seed3)*f6);
			par8+=(double)f7;
			seed0+=(double)(MathHelper.sin(seed3)*f6);
			seed4*=0.7F;
			seed4+=f4*0.05F;
			seed3+=f3*0.05F;
			f4*=0.8F;
			...
			if (flag||random.nextInt(4)!=0){
				...
				double d10=(double)(seed6-seed5);
				double d11=(double)(seed2+2.0F+16.0F);
				...
				if (par6>=blockCoordX-16.0D-d6*2.0D&&seed0>=blockCoordZ-16.0D-d6*2.0D&&par6<=blockCoordX+16.0D+d6*2.0D&&seed0<=blockCoordZ+16.0D+d6*2.0D){
					...
					if (!flag1){
						for(int posX=minX; posX<maxX; ++posX){
							double d12=((double)(posX+x*16)+0.5D-par6)/d6;

							for(int posZ=minZ; posZ<maxZ; ++posZ){
								double d13=((double)(posZ+z*16)+0.5D-seed0)/d6;
								int arrayIndex=(posX*16+posZ)*128+maxY;
								boolean flag2=false;

								if (d12*d12+d13*d13<1.0D){
									for(int posY=maxY-1; posY>=minY; --posY){
										double d14=((double)posY+0.5D-par8)/d7;

										if ((d12*d12+d13*d13)*(double)field_75046_d[posY]+d14*d14/6.0D<1.0D){
	...
	protected void recursiveGenerate(World world, int x, int z, int par4, int par5, byte[] blocks){
		if (rand.nextInt(50)==0){
			double d0=(double)(x*16+rand.nextInt(16));
			double d1=(double)(rand.nextInt(rand.nextInt(28)+8)+10);
			double d2=(double)(z*16+rand.nextInt(16));
	...
	protected void digBlock(byte[] blocks, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop){
		...
		if (block==ModBlocks.umberstone.blockID||block==filler||block==top||true){ // umm, seems to work and make big ravines? eh, whatever
			if (y<10){
